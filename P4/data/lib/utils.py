...
def compute_val_loss(net, val_loader, loss_function, supports, device, epoch):
    ...
    with torch.no_grad():
        ...
        for index, (val_r, val_t) in enumerate(val_loader):
            if temp % 2 == 0 and temp<=274 and temp>=2:
            # if temp % 2 == 0 and temp < 162:
...
def predict(net, test_loader, supports, device):
    ...
    with torch.no_grad():
        ...
        for index, ( test_r, test_t) in enumerate(test_loader):
            if temp % 2 == 0 and temp>=2 and temp <=276:
            #if temp % 2 == 0 and temp < 162:
...
def evaluate(net, test_loader, true_value, supports, device, epoch_):
    ...
    with torch.no_grad():
        ...
        #true_value=true_value[:4432]
        last_ture = true_value[2576:]
        P4_ture=true_value[:2576]
        temp_ture = P4_ture.reshape(-1, 16, 307, 12)
        #prediction=prediction.reshape(-1,16,358,12)
        #true_value=true_value.reshape(-1,16,358,12)
        ture = []
        for i in temp_ture[1::2]:
            ture.append(i)
        # for i in true_value[1::2]:
        #     ture.append(i)
        ture = np.array(ture)
        # ture=ture.reshape(-1,358,12)
        # true_value=ture[:2208]

        true_value= ture.reshape(-1, 307, 12)
        #true_value = ture.reshape(-1, 358, 12)

        true_value = np.concatenate([true_value, last_ture], 0)
