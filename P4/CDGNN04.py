...
FLAGS = parser.parse_args()#50010,19.80
decay = FLAGS.decay
...
if __name__ == "__main__":
    ...
    if  not load_matrix:
        ...
        A_lst = np.array('./data/PEMS04/PEMS04_adj.npy')
        ...
        A_lst = np.load('./data/PEMS04/PEMS04_adj.npy')
    A = A_lst
    A[np.isnan(A)] = 0.
    A[np.isinf(A)] = 0.
    train_A = A[:832]
    ...
    print('\n\nTraining finished.')
    torch.save(G.state_dict(), './data/PEMS04/hawkes_parament(-1,0,1)')
    G.load_state_dict(torch.load('./data/PEMS04/hawkes_parament(-1,0,1)'))
    fake_event_list=[]
    ...
    fake_event_list[fake_event_list>=0.01]=1
    fake_event_list[fake_event_list<=-0.01]=-1
    fake_event_list[(fake_event_list>0.01)&(fake_event_list<-0.01)]=0
    ...
    for epoch in range(1, epochs + 1):
        ...
        if validation_loss_lst[arg] < 950.0 and learning_rate == 0.001:
